 ...
 * The cast.
 *
 * One creature became five, and they all run on the same rig: a critically
 * damped spring for the body, a four-point Verlet chain for whatever is on
 * top of the head, squash and stretch from vertical velocity, eyes that
 * track and blink on their own schedule. What changes per character is the
 * drawing — the silhouette, the palette, the accessories — and a handful of
 * numbers. That is deliberate: five separate animation loops would be five
 * things to keep in sync, and the physics is the part that makes any of them
 * feel alive.
 *
 * Every one of them is drawn in the same 200x220 box with the eyes at the
 * same height, so the rig never has to know which creature it is moving.
 */
...
interface MoodPose {
  ...
  mouth: number;
  /** Radians the arms rotate outward. */
  arms: number;
  /** Multiplier on idle bob amplitude. */
  liveliness: number;
  /** Where the creature looks when it is not tracking anything. */
  gaze: Vec | null;
}
...
const SEG = 13; // chain segment length in viewBox units
const CHAIN = 4;
...
export interface Species {
  id: SpeciesId;
  name: string;
  /** One line of character, used on the celebration screen. */
  blurb: string;
  body: string;
  ink: string;
  edge: string;
...
export const CAST: Record<SpeciesId, Species> = {
  bit: {
    id: 'bit',
    name: 'Bit',
    blurb: 'Runs the place. Blinks at your cursor.',
    body: '#ffffff',
    ink: '#08080a',
    edge: '#ffffff',
    head: { x: 66, y: 112, w: 68, h: 76, rx: 26 },
    crown: 'caret',
    arms: 'bracket',
    front: (
      <>
        {/* a seam and two vents: enough detail to read as machined, not enough to fuss */}
        <path d="M74 176 H126" stroke="#08080a" strokeWidth="1.4" opacity="0.18" />
        <circle cx="79" cy="182" r="1.6" fill="#08080a" opacity="0.2" />
        <circle cx="85" cy="182" r="1.6" fill="#08080a" opacity="0.2" />
        <rect x="88" y="104" width="24" height="9" rx="4" fill="#ffffff" opacity="0.5" />
      </>
    ),
  },
  pip: {
    id: 'pip',
    name: 'Pip',
    blurb: 'New here too. Cheers loudest.',
    body: '#7ef0b2',
    ink: '#06251a',
    edge: '#7ef0b2',
    head: { x: 61, y: 114, w: 78, h: 74, rx: 37 },
    crown: 'leaf',
    arms: 'mitt',
    front: (
      <>
        <ellipse cx="78" cy="164" rx="7" ry="4.5" fill="#0d3a28" opacity="0.28" />
        <ellipse cx="122" cy="164" rx="7" ry="4.5" fill="#0d3a28" opacity="0.28" />
      </>
    ),
  },
  byte: {
    id: 'byte',
    name: 'Byte',
    blurb: 'Keeps the streak. Never sleeps.',
    body: '#f6c66b',
    ink: '#2a1c05',
    edge: '#f6c66b',
    head: { x: 58, y: 118, w: 84, h: 68, rx: 14 },
    crown: 'none',
    arms: 'bracket',
    behind: (
      <>
        {/* headphone cans, so the silhouette is recognisable at 40px */}
        <rect x="44" y="132" width="16" height="34" rx="8" fill="#f6c66b" />
        <rect x="140" y="132" width="16" height="34" rx="8" fill="#f6c66b" />
        <path d="M52 134 Q100 86 148 134" stroke="#f6c66b" strokeWidth="6" fill="none" strokeLinecap="round" />
      </>
    ),
  },
  nib: {
    id: 'nib',
    name: 'Nib',
    blurb: 'Marks the mistakes. Means well.',
    body: '#ff8080',
    ink: '#2d0a0a',
    edge: '#ff8080',
    head: { x: 71, y: 118, w: 58, h: 70, rx: 18 },
    crown: 'none',
    arms: 'mitt',
    behind: <path d="M100 86 L120 128 H80 Z" fill="#ff8080" />,
    front: <path d="M100 106 v10" stroke="#2d0a0a" strokeWidth="2" opacity="0.35" />,
  },
  loop: {
    id: 'loop',
    name: 'Loop',
    blurb: 'Brings back what you missed.',
    body: '#c9c9d4',
    ink: '#101014',
    edge: '#c9c9d4',
    head: { x: 68, y: 114, w: 64, h: 72, rx: 32 },
    crown: 'none',
    arms: 'none',
    behind: (
      <g opacity="0.9">
        <circle cx="100" cy="150" r="52" fill="none" stroke="#c9c9d4" strokeWidth="7" strokeDasharray="18 10" />
      </g>
    ),
  },
};

export const CAST_LIST: Species[] = [CAST.bit, CAST.pip, CAST.byte, CAST.nib, CAST.loop];

/** A stable character for a given seed, so the same lesson gets the same face. */
export function speciesFor(seed: number): Species {
  return CAST_LIST[Math.abs(Math.floor(seed)) % CAST_LIST.length];
}
...
export function Mascot({
   ...
   * the cast watch the room rather than a pointer that is not there.
   */
  ...
  return (
    <svg
      ref={svgRef}
      className={`bit bit--${kind.id} ${className}`}
      width={size}
      height={size * 1.1}
      viewBox="0 0 200 220"
      role="img"
      aria-label={`${kind.name} the mascot, looking ${mood}`}
      onClick={() => {
        pokeRef.current = 2.4;
        onPoke?.();
      }}
    ...
    >
      ...
      <g ref={rootRef}>
        ...
        <g ref={sparkRef} opacity="0">
          ...
          <path d="M150 78 l1.6 -4.4 l1.6 4.4 l4.4 1.6 l-4.4 1.6 l-1.6 4.4 l-1.6 -4.4 l-4.4 -1.6 z" fill={kind.edge} opacity="0.55" />
        </g>

        <g ref={zzzRef} opacity="0">
          <text x="146" y="92" fontFamily="var(--font-mono)" fontSize="16" fill={kind.edge} opacity="0.75">z</text>
          <text x="158" y="76" fontFamily="var(--font-mono)" fontSize="12" fill={kind.edge} opacity="0.5">z</text>
        </g>
        ...
        <g ref={bodyRef}>
          ...
          {kind.arms === 'bracket' ? (
            ...
            <>
              <path ref={armLRef} d="M62 150 L48 158" stroke={kind.edge} strokeWidth="12" strokeLinecap="round" fill="none" />
              <path ref={armRRef} d="M138 150 L152 158" stroke={kind.edge} strokeWidth="12" strokeLinecap="round" fill="none" />
            </>
          ) : (
            <>
              <path ref={armLRef} d="M0 0" fill="none" />
              <path ref={armRRef} d="M0 0" fill="none" />
            </>
          )}

          <rect x={kind.head.x} y={kind.head.y} width={kind.head.w} height={kind.head.h} rx={kind.head.rx} fill={kind.body} />
          {/* a soft top-light keeps the flat fill from looking like a sticker */}
          <rect
            x={kind.head.x}
            y={kind.head.y}
            width={kind.head.w}
            ...
            fill={`url(#${uid}-sheen)`}
          />
          {/* two feet, so it stands on the shadow rather than floating over it */}
          <rect x={kind.head.x + 10} y={kind.head.y + kind.head.h - 4} width="18" height="8" rx="4" fill={kind.body} />
          <rect x={kind.head.x + kind.head.w - 28} y={kind.head.y + kind.head.h - 4} width="18" height="8" rx="4" fill={kind.body} />
          {kind.front}
          ...
          <g ref={eyeLRef}>
            ...
            <path ref={lidLRef} d="M75 145 Q84 138 93 145" stroke={kind.ink} strokeWidth="3"
          ...
          <g ref={eyeRRef}>
            ...
            <path ref={lidRRef} d="M107 145 Q116 138 125 145" stroke={kind.ink} strokeWidth="3"
              strokeLinecap="round" fill="none" opacity="0" />
          </g>

          <path ref={mouthRef} d="M87 166 Q100 171 113 166" stroke={kind.ink} strokeWidth="3.4"
...
export function Bit(props: Omit<Parameters<typeof Mascot>[0], 'species'>) {
  return <Mascot {...props} species="bit" />;
}
